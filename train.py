...
def SaveTrainingLog(model,history):
    import json
    
    acc4file=str(round(history.history['val_accuracy'][-1],4))
    loss4file=str(round(history.history['val_loss'][-1],4))
    
    os.makedirs(os.getcwd()+'\\training_log\\'+f'acc_{acc4file}___loss_{loss4file}')
    path=os.getcwd()+'\\training_log\\'+f'acc_{acc4file}___loss_{loss4file}\\'+f'acc_{acc4file}___loss_{loss4file}'
...
def y2numbers(y):
    ...
    for name in y:
        if name in seen:
            y_transformed.append(seen[name])
        else:
            seen[name]=class_id
            class_id+=1
            y_transformed.append(seen[name])
